...
- (void)createButton
{
    for (int i = 0; i< 5; i++)
    {
        ...
        [selectedButton setBackgroundImage:[UIImage imageNamed:@"a_1_bg.png"] forState:UIControlStateNormal];
...
-(void) touchButton:(id)sender {
    ...
    if( delegate != nil && [delegate respondsToSelector:@selector(tabWasSelected:)]) {
        ...
        if (selectedButton) {
            [selectedButton setBackgroundImage:[UIImage imageNamed:@"a_1_bg.png"] forState:UIControlStateNormal];
            [selectedButton release];
            NSString *imgStr = [[NSString alloc] initWithFormat:@"a_1_b%d.png",selectedButton.tag-100+1];
            selectedButton.item_imageView.image = [UIImage imageNamed:imgStr];
            selectedButton.item_title.textColor = [UIColor blueColor];
        
        }
        
        selectedButton = [((DIYButton *)sender) retain];
        [selectedButton setBackgroundImage:[UIImage imageNamed:@"a_1_bg_se.png"] forState:UIControlStateNormal];
        NSString *imgStr = [[NSString alloc] initWithFormat:@"a_1_b%d_se.png",selectedButton.tag-100+1];
        selectedButton.item_imageView.image = [UIImage imageNamed:imgStr];
        selectedButton.item_title.textColor = [UIColor whiteColor];
